...
- [ ] Top Header: Display "D1" logo badge with "CMS_ADMIN" tag, "Open Orders" toggle switch (active green indicator), status indicator, and user profile avatar.
...
- [ ] Main Content Area (Customer List / User View):
  ...
  - Action items per row: "Mark as Influencer" pill button, "Ban" (destructive action), "Edit", "Settle".
- [ ] Styling & Aesthetics: Matches screenshot color palette (soft salmon/coral highlights `#F05A4A`/`#E76F51`, soft gray container backgrounds, clean typography, rounded card layouts).
- [ ] Unit & Component Tests: Write Vitest + React Testing Library component tests covering navigation rendering, section collapsing/expanding, search filtering, and user table action callbacks.
...
- [ ] Build succeeds via `pnpm build`
- [ ] Visual verification of D1 Admin Layout matching provided image design
