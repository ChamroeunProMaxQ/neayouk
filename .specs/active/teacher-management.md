...
- `packages/contracts/src/resource.enum.ts` — Add `TEACHER = "teacher"`.
...
- `apps/api/src/teacher/teacher.permission.ts` — CASL definitions.
- `apps/api/src/teacher/teacher.module.ts` — Module definition.
- `apps/api/src/app.module.ts` — Register `TeacherModule`.
- `apps/api/src/teacher/teacher.service.spec.ts` — All-condition unit test suite.
- `apps/api/test/teacher.e2e-spec.ts` — All-condition E2E integration test suite.

### 3.3 `apps/web`
- `apps/web/src/features/teachers/`
  - `hooks/use-teachers-infinite-query.ts` — Infinite query hook with debounced search and URL filters.
  - `hooks/use-teachers-query.ts` — Standard query hook for dropdown selects (e.g. assigning to class).
  - `hooks/use-teacher-detail-query.ts` — Single teacher detail hook.
  - `hooks/use-teacher-mutations.ts` — Create, update, delete mutations with cache invalidation.
  - `components/teacher-list-table.tsx` — Responsive data table with search, status filter, user badge, assigned classes count, and actions.
...
- `apps/web/src/features/classes/components/class-form-dialog.tsx` — Add teacher selection dropdown to assign teacher to class.
- `apps/web/src/features/admin/components/admin-sidebar.tsx` — Wire Teacher sidebar route under `Users` $\rightarrow$ `Teachers & Staff` (`/teachers`).
...
- [x] Class form dialog supports selecting and assigning a teacher.
- [x] Unit tests pass via `pnpm test` (all 6 condition categories covered).
- [x] E2E tests pass via `pnpm test:e2e`.

---

## 5. Future Roadmap & HR Payroll Integration
